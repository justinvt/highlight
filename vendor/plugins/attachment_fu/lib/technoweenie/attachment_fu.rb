module Technoweenie # :nodoc:
  module AttachmentFu # :nodoc:
    ...
    module InstanceMethods
      ...
      def thumb_sizes
        owner_class = self.class.reflect_on_all_associations(:belongs_to).reject{|o| o.class_name.gsub(/[^a-zA-Z]/,'') =~ /Image/ }[0]
      ...
      def thumb_exists?(thumbnail)
        !(thumbnails.select{|t| t.thumbnail.to_sym == thumbnail.to_sym}[0].nil?)
      end
      
      def create_if_blank(thumbnail, options={})
        thumb = find_or_initialize_thumbnail(thumbnail)
        if options[:recreate]
          resize_image_or_thumbnail!(thumb)
        end
        unless thumb.nil?
          data = open(public_filename).read
          tmp_file = File.join(RAILS_ROOT, 'tmp', filename)
          size = thumb_sizes.to_a.select{|s| s[0].to_sym == thumbnail.to_sym }[0][1]
          f = File.open(tmp_file, 'w+')
          f.puts data
          f.close
          create_or_update_thumbnail(tmp_file, thumbnail, size)
          File.delete(tmp_file)
        end
        find_or_initialize_thumbnail(thumbnail)
      end
      ...
      protected
        ...
        def after_process_attachment
          if @saved_attachment
            if respond_to?(:process_attachment_with_processing) && thumbnailable? && !attachment_options[:thumbnails].blank? && parent_id.nil?
              ...
              thumb_sizes.each { |suffix, size| create_or_update_thumbnail(temp_file, suffix, *size) }
            end
            save_to_storage
            @temp_paths.clear
        ...
        def destroy_thumbnails(size=nil)
          if size.nil?
            self.thumbnails.each { |thumbnail| thumbnail.destroy } if thumbnailable?
          else
            self.thumbnails.select{|t| t.attributes["thumbnail"] == size.to_s}.each { |thumbnail| thumbnail.destroy } if thumbnailable?
          end
        end
    end
  end
